...
French translation of `lead-generation-scraping-apis`, published as `github.com/automationbyexperts/apis-scraping-generation-de-leads`
(local folder `all_public_actors/APIFY_GITHUB/apis-scraping-generation-de-leads`). Same affiliate model: every Apify link
...
- A regional group sits **first** in `groups` (`france-francophonie`: France, Belgique, Suisse et Québec), so a local tool is
